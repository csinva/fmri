...
def make_character_ds(grids, trfiles):
    ...
    for st in stories:
        ...
        fixed_grtranscript = [(s, e, map(int, c.split(",")))
                              for s, e, c in grtranscript if c]
...
def make_character_model(dss):
    ...
    """
    stories = dss.keys()
    storychars = dict([(st, np.unique(np.hstack(ds.data)))
                      for st, ds in dss.iteritems()])
    total_chars = sum(map(len, storychars.values()))
    char_inds = dict()
    ncharsdone = 0
    for st in stories:
        char_inds[st] = dict(zip(storychars[st], range(
            ncharsdone, ncharsdone+len(storychars[st]))))
